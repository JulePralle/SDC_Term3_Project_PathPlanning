...
vector<vector<double>> get_Trajectory(bool too_close, int id_car_nearest_front, int lane, json j,
  ...
  if(id_car_nearest_front != -1){
    vector<double> data_car_nearest_front = j["sensor_fusion"][id_car_nearest_front];
    double vx = data_car_nearest_front[3];
    double vy = data_car_nearest_front[4];
    speed_car_nearest_front = sqrt(vx*vx + vy*vy);
  }
  ...
  if(too_close)
  {
    ...
    {
      if(speed_car_nearest_front <= 49.5)
      {
        ref_vel = speed_car_nearest_front;
        cout<<"Follow car in front"<<endl;
      }
    }
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
            ...
            if(target_lane == lane)
            {
              ...
              if (id_nearest_car_front != -1)
              {
                ...
                if (min_dist_front < 30)
                {
                  ...
                  // lane keep cost
                  next_vals  = get_Trajectory(too_close, id_nearest_car_front, lane, j[1],
                  ...
                  cost_lk = get_Cost(next_x_vals, next_y_vals);
                  ...
                  if(lane > 0)
                  {
                    ...
                    if(able_to_change)
                    {
                      next_vals  = get_Trajectory(too_close, id_nearest_car_front, lane-1, j[1],
                        map_waypoints_x, map_waypoints_y, map_waypoints_s);

                      next_x_vals = next_vals[0];
                      next_y_vals = next_vals[1];

                      cost_lcl = get_Cost(next_x_vals, next_y_vals);
                    }
                  ...
                  if(lane < 2)
                  {
                    ...
                    if(able_to_change)
                    {
                      ...
                      cost_lcr = get_Cost(next_x_vals, next_y_vals);
            ...
            next_vals  = get_Trajectory(too_close, id_nearest_car_front, target_lane, j[1],
              map_waypoints_x, map_waypoints_y, map_waypoints_s);
            next_x_vals = next_vals[0];
            next_y_vals = next_vals[1];
