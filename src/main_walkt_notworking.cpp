        	...
        	// Main car's localization Data
          	...
          	for (int i = 0; i < sensor_fusion.size(); i++)
          	{
                ...
                if(d < (2+4*lane+2) && d > (2+4*lane-2))
                {
                    ...
                    check_car_s +=((double)prev_size*.02*check_speed); // if using previous points can project s value output
                    // check s values greater than mine an s gap
                    if((check_car_s > car_s)&&((check_car_s-car_s) < 30))
                    {
                        // Here add some logic, lower reference velocity to not crash into other cars, could we always flag to try to chnage lanes
                        //ref_vel = 29.5;
                        too_close = true;
